	...
	public double calculer(Grille g) {
		...
		for(int i = 0; i < g.getTaille(); i++){
			for (int j = i+1; j < g.getTaille(); j++){
				cpt += Math.pow(distance(((Croix)g.getCroix().get(i)),((Croix)g.getCroix().get(j))),-this.p);
			}
		}
		return cpt;
	}
	...
	public double calculerdelta(Grille g, MutationLH m){
		...
		for (int i = 0; i < g.getTaille(); i++){
			if (i != m.getIndex() && i != m.getCriticalIndex()){
				cpt -= Math.pow(distance(((Croix)g.getCroix().get(i)),((Croix)g.getListe().get(m.getIndex()))),-this.p);
				cpt += Math.pow(distance(((Croix)g.getCroix().get(i)),next1),-this.p);
				cpt -= Math.pow(distance(((Croix)g.getCroix().get(i)),((Croix)g.getListe().get(m.getCriticalIndex()))),-this.p);
				cpt += Math.pow(distance(((Croix)g.getCroix().get(i)),next2),-this.p);
			}
		}
